...
export default function Createsale() {
  ...
  const updateCartQty = (id, newQty, maxQty) => {
    const val = Math.max(1, Math.min(newQty, maxQty)); // Prevent < 1 or > stock
    if (newQty > maxQty) toast.warning(`Only ${maxQty} units available`);
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-end">
                <input 
                  type="number"
                  value={item.cartQty}
                  onChange={(e) => updateCartQty(item.id, parseInt(e.target.value) || 0, item.qty)}
